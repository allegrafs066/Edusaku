...
const InputBox: React.FC<{
  ...
  centered?: boolean;
}> = ({ dark, input, isChatting, onInputChange, onSend, centered }) => {
  const inputBg = dark
    ? 'bg-gray-800 border-gray-700 text-white placeholder-gray-500 focus:border-blue-500'
    : 'bg-white border-slate-200 text-slate-900 placeholder-slate-400 focus:border-blue-400';

  return (
    <div className={centered ? 'w-full max-w-2xl mx-auto' : 'relative max-w-4xl mx-auto w-full'}>
      <div className="relative">
        <input
          ...
          className={`w-full rounded-2xl py-4 pl-5 pr-14 border text-sm outline-none transition-all focus:ring-2 focus:ring-blue-500/20 shadow-sm ${inputBg}`}
          autoFocus={centered}
        />
        <button
          onClick={onSend}
          disabled={!input.trim() || isChatting}
          className="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 disabled:dark:bg-gray-600 text-white rounded-xl transition-colors"
        >
          <Send size={16} />
        </button>
      </div>
